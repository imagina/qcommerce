...
export default {
  ...
  computed: {
    crudData() {
      return {
        crudId: this.crudId,
        entityName: config("main.qcommerce.entityNames.weightClasses"),
        apiRoute: 'apiRoutes.qcommerce.lengthClasses',
        permission: 'icommerce.lengthclasses',
        create: {
          title: this.$tr('icommerce.cms.sidebar.newLengthClasses'),
        },
        computed: {
          isValid() {
        ...
        read: {
          columns: [
            ...
            {name: 'title', label: this.$tr('isite.cms.form.name'), field: 'title', align: 'left'},
            {
              name: 'unit', label: this.$tr('icommerce.cms.form.unit'), field: 'unit', style: 'width: 50px',
              align: 'left',
            },
            {name: 'value', label: this.$tr('isite.cms.label.value'), field: 'value', align: 'right'},

            {
              name: 'default', label: this.$tr('isite.cms.form.default'),
        ...
        update: {
          title: this.$tr('icommerce.cms.sidebar.updateLengthClasses'),
          requestParams: {}
        },
        delete: true,
        //Form
